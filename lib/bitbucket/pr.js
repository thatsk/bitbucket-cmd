     ...
     function getPullRequests(options, callback){
       ...
       var query = '?fields=%2Bvalues.reviewers.username,values.id,values.title,values.state,values.destination.branch.name,values.source.branch.name,values.author.username&q=state="open"';
            if (options.list) {
              ...
              query = '?fields=%2Bvalues.reviewers.username,values.id,values.title,values.state,values.destination.branch.name,values.source.branch.name,values.author.username&pagelen=100&q=state="open" AND author.username="'+user+'"';
            }              
            if (options.merged) {
              query = '?q=state+%3D+%22MERGED%22&fields=%2Bvalues.reviewers.username,values.id,values.title,values.state,values.destination.branch.name,values.source.branch.name,values.author.username&pagelen=100';
            }       
            if (options.review) {
              query = '?fields=%2Bvalues.reviewers.username,values.id,values.title,values.state,values.destination.branch.name,values.source.branch.name,values.author.username&pagelen=100&q=reviewers.username="'+conf.username+'" AND state="OPEN"';
            }       
            if (options.__branch) {
              query = '?fields=%2Bvalues.reviewers.username,values.id,values.title,values.state,values.destination.branch.name,values.source.branch.name,values.author.username&pagelen=100&q=source.branch.name="'+options.__branch+'" AND state="OPEN"';
            }
            if (options.__pr_id) {
              query = '?fields=%2Bvalues.reviewers.username,values.id,values.title,values.state,values.destination.branch.name,values.source.branch.name,values.author.username&pagelen=100&q=id='+ options.__pr_id;
            }
