...
define([
    ...
    'simple-git',
     ...
     'openurl',
     'commander'  
], function (request, Table, moment, path, config, git, colors, openurl, program) {
     ...
     function getPullRequests(options, callback){
       ...
       var query = '?fields=%2Bvalues.reviewers.username,values.id,values.title,values.state,values.destination.branch.name,values.source.branch.name,values.author.username&q=state="open"';
            if (options.list) {
              query = '?fields=%2Bvalues.reviewers.username,values.id,values.title,values.state,values.destination.branch.name,values.source.branch.name,values.author.username&pagelen=50&q=state="open" AND author.username="'+conf.username+'"';
            }              
            if (options.merged) {
            ...
            if (options.by) {
              query = '?fields=%2Bvalues.reviewers.username,values.id,values.title,values.state,values.destination.branch.name,values.source.branch.name,values.author.username&pagelen=50&q=author.username="'+options.by+'" AND state="OPEN"';
            }       
    ...
    var pullrequest = {
            ...
            request
              ...
              if(res.body && res.body.destination && res.body.destination.branch && res.body.destination.branch.name){
                ...
                git(conf.currentPath).checkout(destBranch, function(err, info){
                  ...
                  if(err){
                    return console.log('error checking out to destination branch '+destBranch);
                  }                  
      ...
      create: function (options, cb) {
        ...
        git(conf.currentPath).status(function (err, info) {
          ...
          git(conf.currentPath).log(function(err, res){
            console.log('getting latest commit');
            if(err){              
              console.log('error getting git log');
              return cb();
            }
            var latestCommitMsg =  res.latest.message;
